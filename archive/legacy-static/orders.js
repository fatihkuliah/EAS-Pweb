      ...
      (order) => `
        <div class="order-history">
          ...
          <div class="history-action">
            ...
            <a href="detail-order.html?id=${order.id}" class="order-back text-decoration-none">Detail</a>
            <button onclick="invoice('${order.id}')" class="order-back">Invoice</button>
            ${order.status == 'Selesai' ? `<a href="review.html?id=${order.id}" class="order-back text-decoration-none">Review</a>` : ''}
          </div>
        </div>
      `
    )
